...
export const ReturnDataTable = ({loading, allItems,state,action}) => {
    ...
    const handleChange = (e, item) => {
        ...
        if (e.target.checked) {
            action({
                ...
                payload: state.selectedItems.concat([item])
            })
        } else {
            action({
                type: "SET_SELECTED_ITEM",
                payload: state.selectedItems.filter(elem => {
    ...
    const controlInput = (e,max,id) => {
        ...
        action({
            type: "SET_SELECTED_ITEM",
            payload: state.selectedItems.map(elem => {
                if (elem.id === id) {
                    return {
                        ...elem,
    ...
    const toggleAll = (e) => {
        ...
        if (e.target.checked) {
            action({
                type: "SET_SELECTED_ITEM",
                payload: state.selectedItems.concat(allItems.filter(elem => {
    ...
    useEffect(() => {
        setSelectedIds(state.selectedItems?.map(item => {
            return item.id
        }));
    }, [state.selectedItems])
    if (loading) {
                ...
                <div className="table-responsive">
                    <table className="table align-middle">
                        <thead className="align-middle">
                            <tr>
                                <th className="cut-text fw-medium"><input type="checkbox" checked={state.selectedItems?.length === allItems?.length} onChange={toggleAll} /></th> 
                                ...
                                <th className="cut-text fw-medium"><Lang>Total(dh)</Lang></th> 
                            </tr>
                        </thead>
                        <tbody >
                            {allItems?.map(item => {
                                    return (
                                        <tr key={item.id}>
                                            ...
                                            <td className="cut-text fw-normal">{item.unitPrice} dh</td>
                                            ...
                                            <td className="cut-text fw-normal">{item.total} dh</td>
                                        </tr>
                                    )
                                })
                            }
